...
import { applyFirstShiftReport, compilePackage, createOrganizationFromEvidence, createTeamLaunchpad, evidenceHash, knowledgeEvidence, launchAuthority, markPrimaryRoleInstalled, mergeOrganization, organizationFromInterview, parseImportedPackage, repositoryEvidence, roleSkill, verifyFirstShift, } from "../lib/organization.js";
...
import { evaluateOrganizationReadiness, readinessMarkdown } from "../lib/readiness.js";
import { providerEvaluationReportMarkdown, scoreProviderEvaluation, } from "../lib/provider-evaluation.js";
...
function sameAsOrAncestorOf(candidate, target) {
...
async function updateManagedPackageArtifacts(destination, existingFiles, updates) {
    ...
    try {
        ...
        if (archiveMoved) {
            try {
                await rename(backupArchive, archiveLocation);
            }
            catch (rollbackError) {
                rollbackErrors.push(String(rollbackError));
            }
        ...
        for (const path of [...installedPaths].reverse()) {
            try {
                await rm(resolve(destination, path), { force: true });
            }
            catch (rollbackError) {
                rollbackErrors.push(String(rollbackError));
            }
...
async function readPackagedOrganization(packageFolder) {
    const graphLocation = resolve(packageFolder, "graph.json");
    try {
        const graph = JSON.parse(await readBoundedUtf8File(graphLocation, maxPackagedGraphBytes, "package graph"));
...
async function readImportableOrganization(packageFolder) {
    const graphLocation = resolve(packageFolder, "graph.json");
    try {
        const graph = JSON.parse(await readBoundedUtf8File(graphLocation, maxPackagedGraphBytes, "imported package graph"));
        return parseImportedPackage(graph).organization;
    }
    catch (error) {
...
async function ensureUnusedPackageDestination(destination, packageFolders, operation) {
    ...
    try {
        const destinationStats = await stat(destination);
        if (!destinationStats.isDirectory())
            throw new Error(`${operation} output exists and is not a folder: ${destination}`);
...
async function generatedFilesIn(folder, current = folder) {
    const found = [];
    let entries;
    try {
    ...
    for (const entry of entries) {
        ...
        if (entry.isDirectory())
            found.push(...await generatedFilesIn(folder, location));
...
async function writeOrganizationPackage(destination, organization) {
    const files = compilePackage(organization);
    const map = renderOrganizationMap(organization);
    const packageFiles = { ...files, "organization-map.html": map };
    ...
    try {
        if (await pathExists(destination)) {
            const destinationStats = await stat(destination);
            if (!destinationStats.isDirectory())
                throw new Error(`package output exists and is not a folder: ${destination}`);
        ...
        if (oldArchiveMoved) {
            try {
                await rename(archiveBackup, archiveLocation);
            }
            catch (rollbackError) {
                rollbackErrors.push(String(rollbackError));
            }
...
async function runMerge(args) {
    ...
    if (!baseInput)
        throw new Error("merge requires --package <base-folder>.");
    const additionalInputs = argumentValues(args, "--with");
    if (!additionalInputs.length)
        throw new Error("merge requires at least one --with <team-folder>.");
    ...
    for (const incoming of organizations.slice(1))
        merged = mergeOrganization(merged, incoming);
    merged = {
        ...merged,
...
async function runRefresh(args) {
    ...
    let refreshed = rebuilt[0].organization;
    for (const unit of rebuilt.slice(1))
        refreshed = mergeOrganization(refreshed, unit.organization);
    refreshed = {
        ...refreshed,
        name: existing.name,
...
function managedArtifactByteLimit(path) {
    if (path === "graph.json")
        return maxPackagedGraphBytes;
...
async function readActualPackageFiles(packageFolder) {
    const actualFiles = {};
    const paths = await generatedFilesIn(packageFolder);
    await mapConcurrent(paths, 4, async (path) => {
        try {
            actualFiles[path] = await readBoundedUtf8File(resolve(packageFolder, path), managedArtifactByteLimit(path), "managed package artifact");
        }
...
function replayableGitInput(input, baseDirectory) {
    ...
    try {
        ...
        if (!/^[^@\s]+@[^:\s]+:.+/.test(repositoryInput))
            repository = resolve(baseDirectory, repositoryInput);
    }
    if (marker <= 0)
        return repository;
...
function createSourceReplayPlan(options) {
    ...
    const urls = options.urls.map((input) => {
        try {
            ...
            if (parsed.search)
                limitations.push(`Signed or query-bearing URL must be supplied again: ${sanitizeExternalLocator(input)}`);
        }
        catch {
...
async function buildOrganizationFromSourcePlan(plan, options = {}) {
    let baseStats;
    try {
        baseStats = await stat(plan.baseDirectory);
    }
    catch {
        throw new Error(`I can’t find the source-plan base folder: ${plan.baseDirectory}`);
    }
    if (!baseStats.isDirectory())
        throw new Error(`The source-plan base must be a folder: ${plan.baseDirectory}`);
    const databaseUrlsFromEnvironment = plan.databaseUrlEnvs.map((name) => {
        const value = process.env[name];
        if (!value)
            throw new Error(`database URL environment variable ${name} is not set.`);
        return value;
    });
    ...
    const [localScan, externalScan, databaseQueryScans] = await Promise.all([
        ...
        mapConcurrent(plan.databaseQueryManifests, databaseQueryManifestConcurrency, async (inputPath) => {
            ...
            let content;
            try {
                content = await readBoundedUtf8File(location, maxDatabaseQueryManifestBytes, "database query manifest");
            }
            catch (error) {
            ...
            try {
                ...
                throw new Error(`Database query manifest is not valid JSON: ${location}`);
            }
    ...
    const evidence = knowledgeEvidence(scan.sources.map((source) => (!source.sourceConnector || source.sourceConnector === "local"
    ...
    return {
        ...
        organization: {
            ...
            ingestion: {
                schemaVersion: "1.0",
                status: scan.skipped.length ? "complete-with-warnings" : "complete",
                recordedAt: new Date().toISOString(),
...
async function runOnboard(args) {
    ...
    try {
        ...
        throw new Error(`I can’t find the base folder: ${baseDirectory}`);
    }
    if (!baseStats.isDirectory())
        throw new Error(`The onboarding base must be a folder: ${baseDirectory}`);
    ...
    try {
        ...
        let organization = built.organization;
        const reviewInput = argumentValue(args, "--review");
        if (reviewInput) {
            const reviewLocation = resolve(baseDirectory, reviewInput);
            let reviewContent;
            try {
                reviewContent = await readBoundedUtf8File(reviewLocation, maxReviewBytes, "organization review");
            }
            catch (error) {
                throw new Error(`Could not read the organization review file: ${error instanceof Error ? error.message : reviewLocation}`);
            }
            let reviewValue;
            try {
                reviewValue = JSON.parse(reviewContent);
            }
            catch {
                throw new Error(`The organization review file is not valid JSON: ${reviewLocation}`);
            }
        ...
        if (scan.skipped.length) {
            ...
            if (scan.skipped.length > 10)
                output.write(`    - …and ${scan.skipped.length - 10} more\n`);
        }
        ...
        if (hasFlag(args, "--no-open") || hasFlag(args, "--yes"))
            output.write("  Map opening skipped.\n\n");
        else {
            openMap(mapLocation);
            output.write("  Opening the organization map in your browser…\n\n");
        }
    }
    finally {
        rl.close();
...
function readinessArtifactsCurrent(files, report) {
    try {
...
function printReadinessIssues(report) {
    report.checks
    ...
    if (hidden > 0)
        output.write(`  - …and ${hidden} more review item${hidden === 1 ? "" : "s"}\n`);
}
...
async function runInstall(args) {
    ...
    if (hasFlag(args, "--no-open"))
        output.write("  Provider opening skipped.\n\n");
    else {
        openMap(providerUrl);
        output.write(`  Opening ${onboardingProviderLabel(requestedProvider)} in your browser…\n\n`);
    }
}
function printProviderEvaluationFailures(report) {
...
async function runEvaluate(args) {
    ...
    if (!resultsInput)
        throw new Error("evaluate requires --results <responses.json>.");
    const organization = await readPackagedOrganization(packageFolder);
    const actualFiles = await readActualPackageFiles(packageFolder);
    const readiness = evaluateOrganizationReadiness(organization, actualFiles);
    if (readiness.status !== "ready") {
    ...
    let resultsContent;
    try {
        resultsContent = await readBoundedUtf8File(resultsLocation, maxProviderEvaluationBytes, "provider evaluation responses");
    }
    catch (error) {
    ...
    let submission;
    try {
        submission = JSON.parse(resultsContent);
    }
    catch {
        throw new Error(`Provider evaluation responses are not valid JSON: ${resultsLocation}`);
    }
...
async function runCheck(args) {
    ...
    const tracedEvidence = localEvidence.filter((item) => (item.sourceBaseDirectory
    ...
    for (const evidence of tracedEvidence) {
        ...
        try {
            ...
            if ((await currentHash) !== evidence.sourceHash)
                changed.push(archiveMarker >= 0 ? physicalPath : path);
        }
        catch (error) {
            if (error.code === "ENOENT")
                missing.push(physicalPath);
            else
                unreadable.push(physicalPath);
        }
    }
    ...
    if (externalEvidence.length || unverifiableEvidence.length) {
        ...
        if (unverifiableEvidence.length)
            counts.set("unmapped", unverifiableEvidence.length);
...
try {
    if (command === "help" || args.includes("--help") || args.includes("-h"))
        ...
        output.write(`DeltaDotta ${cliVersion}\n`);
