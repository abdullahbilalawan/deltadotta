...
function manifestName(source, field, label) {
    const value = requiredString(source, field, label);
    if (value.length > 120)
        throw new Error(`${label}.${field} must be 120 characters or fewer`);
    if (/[\u0000-\u001f\u007f/\\]/.test(value)) {
        throw new Error(`${label}.${field} cannot contain control characters or path separators`);
    }
    return value;
}
export function parseDatabaseKnowledgeManifest(value) {
    const root = record(value, "database query manifest");
    if (root.schemaVersion !== "1.0")
        throw new Error("database query manifest schemaVersion must be \"1.0\"");
    ...
    if (root.connections.length > 10)
        throw new Error("database query manifest supports at most 10 connections");
    let totalQueries = 0;
    ...
    const connections = root.connections.map((value, connectionIndex) => {
        ...
        if (connectionNames.has(name.toLowerCase()))
            throw new Error(`database query manifest contains duplicate connection name: ${name}`);
        connectionNames.add(name.toLowerCase());
        const urlEnv = requiredString(connection, "urlEnv", label);
        if (!/^[A-Za-z_][A-Za-z0-9_]*$/.test(urlEnv))
            throw new Error(`${label}.urlEnv must name an environment variable`);
        if (!Array.isArray(connection.queries) || !connection.queries.length)
            throw new Error(`${label}.queries must contain at least one query`);
        totalQueries += connection.queries.length;
        if (totalQueries > 50)
            throw new Error("database query manifest supports at most 50 queries");
        ...
        const queries = connection.queries.map((value, queryIndex) => {
            ...
            if (queryNames.has(queryName.toLowerCase()))
                throw new Error(`${label} contains duplicate query name: ${queryName}`);
...
function scrubSqlLiterals(sql) {
    ...
    if (quote)
        throw new Error("database knowledge query contains an unterminated quoted value");
    return output;
}
...
export function validateReadOnlyQuery(input) {
    ...
    if (sql.length > 20_000)
        throw new Error("database knowledge query exceeds the 20,000-character limit");
    if (sql.endsWith(";"))
        sql = sql.slice(0, -1).trim();
    if (!sql)
        throw new Error("database knowledge query is empty");
    const scrubbed = scrubSqlLiterals(sql);
    if (/(?:--|\/\*|\*\/|#)/.test(scrubbed))
        throw new Error("database knowledge queries cannot contain SQL comments");
    if (scrubbed.includes(";"))
        throw new Error("database knowledge queries must contain exactly one statement");
    if (!/^\s*(?:select|with)\b/i.test(scrubbed))
        throw new Error("database knowledge queries must start with SELECT or WITH");
    ...
    if (match)
        throw new Error(`database knowledge query contains forbidden operation: ${match[0].toUpperCase()}`);
    ...
    if (unsafeFunction)
        throw new Error(`database knowledge query contains forbidden function: ${unsafeFunction[0].replace(/\s*\($/, "").toUpperCase()}`);
    return sql;
}
...
export function databaseKnowledgeInvocation(databaseUrl, queryName, query, options = {}) {
    const environment = options.environment ?? process.env;
    const maxRows = options.maxRows ?? defaultMaxRows;
    const timeoutMs = options.timeoutMs ?? 10_000;
    if (!Number.isInteger(maxRows) || maxRows < 1 || maxRows > 5_000)
        throw new Error("database knowledge maxRows must be between 1 and 5000");
...
const defaultRunner = (command, args, options) => new Promise((done, reject) => {
...
function parsePostgresRows(stdout, maxRows) {
    const candidate = stdout.split("\n").map((line) => line.trim()).filter(Boolean).find((line) => line.startsWith("["));
    if (!candidate)
        throw new Error("psql returned no JSON row set");
    const value = JSON.parse(candidate);
    if (!Array.isArray(value))
        throw new Error("psql row set was not a JSON array");
    return {
        content: JSON.stringify(value.slice(0, maxRows), null, 2),
        truncated: value.length > maxRows,
    };
}
function parseMysqlRows(stdout, maxRows) {
    const lines = stdout.replace(/\r/g, "").split("\n").filter((line) => line.length);
    if (!lines.length)
        return { content: "No rows returned.", truncated: false };
...
function positiveInteger(value, label, upperBound) {
    if (!Number.isInteger(value) || value < 1 || value > upperBound) {
        throw new Error(`${label} must be between 1 and ${upperBound}`);
    }
    return value;
}
function safeDatabaseError(error, databaseUrl) {
    let message = error instanceof Error ? error.message : String(error);
    const secrets = [databaseUrl];
    try {
        const parsed = new URL(databaseUrl);
        if (parsed.password)
            secrets.push(decodeURIComponent(parsed.password), parsed.password);
    }
    catch {
...
export async function collectDatabaseKnowledge(manifestValue, options = {}) {
    ...
    for (const connection of manifest.connections) {
        ...
        for (const query of connection.queries) {
            ...
            try {
                ...
                if (contentBytes > maxBytesPerQuery)
                    throw new Error(`query output exceeds the ${maxBytesPerQuery}-byte limit`);
                ...
                if (parsed.truncated)
                    skipped.push({ path, reason: `query result truncated at ${invocation.maxRows} rows` });
            }
            catch (error) {
                skipped.push({
                    path,
