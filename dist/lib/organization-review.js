...
function requiredStringList(source, field, label) {
    const value = source[field];
...
function requiredReference(source, field, label) {
    ...
    if (value === null)
        return null;
    if (typeof value !== "string" || !value.trim())
        throw new Error(`${label}.${field} must contain a role title/id or null`);
    return value.trim();
}
...
function parseReview(value) {
    ...
    if (root.schemaVersion !== "1.0")
        throw new Error("review.schemaVersion must be \"1.0\"");
    ...
    if (!Number.isFinite(Date.parse(reviewedAt)))
        throw new Error("review.reviewedAt must be an ISO-8601 date or timestamp");
    ...
    const roles = organization.roles.map((value, index) => {
        const label = `review.organization.roles[${index}]`;
        const role = record(value, label);
        if (typeof role.confirmed !== "boolean")
            throw new Error(`${label}.confirmed must be true or false`);
        return {
    ...
    roles.forEach((role, index) => {
        if (!role.id)
            role.id = missingRoleIds[index];
    });
    const sourceConflicts = (organization.sourceConflicts === undefined ? [] : organization.sourceConflicts);
    if (!Array.isArray(sourceConflicts))
        throw new Error("review.organization.sourceConflicts must be an array");
    ...
    const conflicts = sourceConflicts.map((value, index) => {
        ...
        if (conflictIds.has(id))
            throw new Error(`duplicate reviewed source conflict id: ${id}`);
        conflictIds.add(id);
        const field = conflict.field;
        if (field !== "department" && field !== "reportsTo" && field !== "authority")
            throw new Error(`${label}.field must be department, reportsTo, or authority`);
        if (!Array.isArray(conflict.claims) || conflict.claims.length < 2)
            throw new Error(`${label}.claims must contain at least two claims`);
        ...
        if (typeof conflict.resolution !== "string")
            throw new Error(`${label}.resolution must be a string`);
        if (typeof conflict.resolved !== "boolean")
            throw new Error(`${label}.resolved must be true or false`);
        return {
            id,
    ...
    if (!Array.isArray(ingestionWarnings))
        throw new Error("review.organization.ingestionWarnings must be an array");
    ...
    const warnings = ingestionWarnings.map((value, index) => {
        ...
        if (warningIds.has(id))
            throw new Error(`duplicate reviewed ingestion warning id: ${id}`);
        warningIds.add(id);
        if (typeof warning.acknowledged !== "boolean")
            throw new Error(`${label}.acknowledged must be true or false`);
        return {
            id,
...
export function createOrganizationReviewTemplate(organization) {
    const titleForRole = (id) => organization.roles.find((role) => role.id === id)?.title ?? null;
    return {
        schemaVersion: "1.0",
        instructions: [
...
export function applyOrganizationReview(base, value, options = {}) {
    const review = parseReview(value);
    const ids = new Set();
    const titles = new Set();
    review.roles.forEach((role, index) => {
        if (!role.id)
            throw new Error(`review.organization.roles[${index}].id could not be derived`);
        if (ids.has(role.id))
            throw new Error(`duplicate reviewed role id: ${role.id}`);
        const normalizedTitle = canonicalRoleKey(role.title);
        if (titles.has(normalizedTitle))
            throw new Error(`duplicate reviewed role title: ${role.title}`);
        ids.add(role.id);
    ...
    const resolveRole = (reference, label) => {
        if (reference === null)
            return undefined;
        const id = idByReference.get(reference.toLowerCase()) ?? idByReference.get(canonicalRoleKey(reference));
        if (!id)
            throw new Error(`${label} references unknown role: ${reference}`);
        return id;
    };
    const roles = review.roles.map((role, index) => {
        const evidenceIds = role.evidence.map((reference) => {
            const id = evidenceByReference.get(reference.toLowerCase());
            if (!id)
                throw new Error(`review.organization.roles[${index}].evidence references unknown source: ${reference}`);
            return id;
        });
        ...
        if (reportsTo === role.id)
            throw new Error(`${role.title} cannot report to itself`);
        if (escalatesTo === role.id)
            throw new Error(`${role.title} cannot escalate to itself`);
        return {
            id: role.id,
            title: role.title,
            department: role.department,
    ...
    if (unknownConflictIds.length)
        throw new Error(`review contains unknown source conflict ids: ${unknownConflictIds.join(", ")}`);
    ...
    const sourceConflicts = baseConflicts.map((conflict) => {
        ...
        if (!decision.resolution)
            throw new Error(`source conflict ${conflict.id} is marked resolved but has no resolution`);
        const reviewedRole = review.roles.find((role) => canonicalRoleKey(role.title) === canonicalRoleKey(conflict.roleTitle));
        const normalize = (input) => input.trim().toLowerCase().replace(/\s+/g, " ");
        let canonicalValue;
        if (!reviewedRole) {
    ...
    if (unknownWarningIds.length)
        throw new Error(`review contains unknown ingestion warning ids: ${unknownWarningIds.join(", ")}`);
    ...
    const canonicalReviewHash = evidenceHash(JSON.stringify(createOrganizationReviewTemplate(reviewedOrganization), null, 2));
    return {
