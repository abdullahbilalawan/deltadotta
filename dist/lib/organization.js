...
import { createProviderEvaluationSubmissionTemplate, createProviderEvaluationSuite, providerEvaluationSuiteMarkdown, } from "./provider-evaluation.js";
...
function enforceOrganizationScale(organization, label = "organization") {
...
function enforceUniqueImportedIds(items, label) {
    const seen = new Set();
    items.forEach((item, index) => {
        if (!item.id.trim())
            throw new Error(`${label}[${index}].id must be a non-empty string.`);
        if (seen.has(item.id))
...
function importRole(value, index) {
    ...
    return {
        ...
        review: source.review && typeof source.review === "object"
            ...
            && typeof source.review.reviewedAt === "string"
            ? {
                reviewedBy: source.review.reviewedBy,
                reviewedAt: source.review.reviewedAt,
                sourceHash: typeof source.review.sourceHash === "string"
...
function importSourceConflict(value, index) {
    if (!value || typeof value !== "object" || Array.isArray(value))
        return null;
    const source = value;
    if (typeof source.roleTitle !== "string" || !source.roleTitle.trim())
        return null;
    if (source.field !== "department" && source.field !== "reportsTo" && source.field !== "authority")
        return null;
    ...
    const claims = source.claims.map((value, claimIndex) => {
        if (!value || typeof value !== "object" || Array.isArray(value)) {
        ...
        return {
            ...
            evidenceIds: stringList(claim.evidenceIds, `organization.sourceConflicts[${index}].claims[${claimIndex}].evidenceIds`, organizationStructureLimits.itemsPerRoleField),
        };
    });
    if (claims.length < 2)
        return null;
...
function importIngestionReport(value) {
    if (!value || typeof value !== "object" || Array.isArray(value))
        return undefined;
    const source = value;
    ...
    if (!Number.isInteger(source.sourceCount) || Number(source.sourceCount) < 0)
        return undefined;
    if (!Number.isInteger(source.totalBytes) || Number(source.totalBytes) < 0)
        return undefined;
    if (source.durationMs !== undefined && (!Number.isInteger(source.durationMs) || Number(source.durationMs) < 0))
        return undefined;
    ...
    const warnings = source.warnings.map((value, index) => {
        if (!value || typeof value !== "object" || Array.isArray(value)) {
...
function importSourceReplayPlan(value, index) {
    if (!value || typeof value !== "object" || Array.isArray(value))
        return null;
    const source = value;
    if (typeof source.baseDirectory !== "string" || !source.baseDirectory.trim())
        return null;
    if (typeof source.organizationName !== "string" || !source.organizationName.trim())
        return null;
    if (typeof source.mission !== "string" || !source.mission.trim())
        return null;
    if (source.provider !== "claude" && source.provider !== "claude-code" && source.provider !== "chatgpt" && source.provider !== "codex")
        return null;
    ...
    const httpTokenEnv = typeof source.httpTokenEnv === "string" && /^[A-Za-z_][A-Za-z0-9_]*$/.test(source.httpTokenEnv)
        ? source.httpTokenEnv
        : undefined;
    const databaseUrlEnvs = stringList(source.databaseUrlEnvs, `organization.sourcePlans[${index}].databaseUrlEnvs`, organizationStructureLimits.itemsPerSourcePlanField)
...
export function parseImportedPackage(value) {
    ...
    if (!Array.isArray(source.roles) || !source.roles.length)
    ...
    const roles = source.roles.map((value, index) => {
        ...
        if (!role)
            throw new Error(`organization.roles[${index}] is not a usable role.`);
        return role;
    });
    ...
    if (!Array.isArray(sourceConflictValues))
        throw new Error("organization.sourceConflicts must be an array.");
    ...
    const sourceConflicts = sourceConflictValues.map((value, index) => {
        ...
        if (!conflict)
            throw new Error(`organization.sourceConflicts[${index}] is not a usable conflict.`);
        return conflict;
    });
    ...
    if (!Array.isArray(sourcePlanValues))
        throw new Error("organization.sourcePlans must be an array.");
    ...
    const sourcePlans = sourcePlanValues.map((value, index) => {
        ...
        if (!plan)
            throw new Error(`organization.sourcePlans[${index}] is not a usable source plan.`);
        return plan;
    });
    enforceUniqueImportedIds(sourcePlans, "organization.sourcePlans");
    const ingestion = importIngestionReport(source.ingestion);
    if (source.ingestion !== undefined && !ingestion)
        throw new Error("organization.ingestion is malformed.");
    ...
    const launch = launchSource
        ...
        ? {
            template: launchSource.template,
            provider: launchSource.provider,
            status: launchSource.status,
...
function roleReferenceTitle(organization, reference) {
    if (!reference)
        return "No direct manager";
    return organization.roles.find((role) => role.id === reference)?.title ?? reference;
}
function canonicalRoleTitleKey(title) {
    return canonicalRoleKey(title);
}
function mergeConflictClaims(existing, incoming) {
...
export function mergeOrganization(current, incoming) {
    ...
    for (const overlap of overlaps) {
        ...
        for (const claim of scalarClaims) {
            if (normalizedSourceClaim(claim.field, claim.currentValue)
                === normalizedSourceClaim(claim.field, claim.incomingValue))
                continue;
            addConflict({
        ...
        const collectAuthority = (permissions, evidenceIds) => {
        ...
        for (const claims of authorityClaims.values()) {
            if (!claims.allow.length || !claims.deny.length)
                continue;
    ...
    const mergedWarnings = current.ingestion && incoming.ingestion
        ? Array.from(new Map([...current.ingestion.warnings, ...incoming.ingestion.warnings].map((warning) => [warning.id, warning])).values())
    ...
    const sourcePlans = Array.from(new Map([...(current.sourcePlans ?? []), ...(incoming.sourcePlans ?? [])]
...
function organizationLookup(org) {
    return {
...
export function roleSkill(role, org, artifactSlug, lookup) {
...
export function packageGaps(org) {
    ...
    const sourceCounts = ["codebase", "document", "database"]
...
function fallbackIngestionReport(org) {
...
function ingestionReportMarkdown(report) {
...
function sourceReplayPlansMarkdown(plans) {
...
function splitUtf8(value, maxBytes) {
    ...
    while (start < encoded.length) {
        ...
        if (end === start)
            throw new Error("provider knowledge byte limit is too small for UTF-8 text");
        parts.push(decoder.decode(encoded.subarray(start, end)));
        start = end;
    }
    return parts;
}
function providerKnowledgeRecords(org) {
...
function providerKnowledgeArtifacts(org, provider) {
    ...
    groups.forEach((records, index) => {
        ...
        if (utf8Bytes(content) > limits.maxBytesPerFile)
            throw new Error(`${path} exceeds its provider knowledge byte limit`);
        files[path] = content;
        paths.push(path);
    });
    return { files, paths, maxBytesPerFile: limits.maxBytesPerFile };
}
function providerInstructions(org, provider) {
...
function providerUploadManifest(org, provider, knowledgePaths) {
...
export function compilePackage(org) {
    ...
    const providerIntegrityManifest = {
        ...
        providers: Object.fromEntries(["chatgpt", "claude"].map((provider) => {
...
function cleanDetectedTitle(candidate) {
    return candidate
        .replace(/^["'`[\]{}*#\s]+|["'`[\]{}*#\s]+$/g, "")
        .replace(/\s+/g, " ")
        .replace(/[.;].*$/, "")
...
function roleSignalFromObject(source, allowName = false) {
    ...
    return {
        ...
        claimedScalarFields: [
            ...
            ...(reportsToTitle ? ["reportsTo"] : []),
        ],
        excerpt,
    };
}
function structuredJsonRoleSignals(text) {
...
function structuredDelimitedRoleSignals(text) {
    ...
    if (!normalizedHeader.some((field) => ["jobtitle", "roletitle", "title", "role", "position"].includes(field)))
        return [];
...
function structuredMarkdownTableRoleSignals(text) {
    ...
    for (let index = 0; index < lines.length - 2; index += 1) {
        ...
        if (!headerLine.includes("|")
            ...
            continue;
        const parseRow = (row) => parseDelimitedRow(row.trim().replace(/^\|/, "").replace(/\|$/, ""), "|");
...
function structuredYamlRoleSignals(text) {
    const lines = text.replace(/\r/g, "").split("\n");
    const signals = [];
    let containerIndent;
    let current;
    ...
    for (const rawLine of lines) {
        ...
        if (containerIndent === undefined) {
            ...
            if (container && yamlRoleContainers.has(normalizedField(container[1])))
                containerIndent = indentation;
            continue;
        }
        const listValue = /^-\s+([^:]+)$/.exec(line);
        if (current && activeListField && indentation > activeListIndent && listValue) {
            const values = Array.isArray(current[activeListField]) ? current[activeListField] : [];
        ...
        if (listMapping) {
            ...
            if (!current || !yamlRoleFields.has(normalized))
                continue;
        ...
        if (!yamlRoleFields.has(normalized))
            continue;
...
function markdownRoleField(value) {
...
function markdownRoleSignals(text) {
    ...
    return headings.flatMap((heading, headingIndex) => {
        ...
        if (!title || title.length > 80 || genericHeading.test(title) || /(?:https?:|@|\/|\\)/i.test(title))
            return [];
        ...
        for (const rawLine of block.replace(/\r/g, "").split("\n")) {
            ...
            if (bullet) {
                if (activeField)
                    values.set(activeField, unique([...(values.get(activeField) ?? []), bullet]));
                else
                    unassignedBullets.push(bullet);
                continue;
            }
        ...
        const hasRoleField = ["purpose", "owns", "permissions", "reportsTo", "inputs", "outputs"]
            .some((field) => (values.get(field)?.length ?? 0) > 0);
        if (!hasRoleField && !(roleLikeTitle.test(title) && unassignedBullets.length))
            return [];
...
function plausibleProseRoleTitle(value) {
    const title = cleanDetectedTitle(value.replace(/^the\s+/i, ""));
    if (!title || title.length > 80 || title.split(/\s+/).length > 8 || !proseRoleTitleShape.test(title))
        return "";
    return title;
}
function proseRoleSignals(text) {
...
function enrichProseSignal(signal, text) {
...
export function extractRoleSignals(text) {
    ...
    while ((match = explicit.exec(compact))) {
        ...
        if (/^owner$/i.test(match[1]) && !plausibleProseRoleTitle(title))
            continue;
        matches.push(title);
    }
    const knownRoles = [
    ...
    matches.forEach((candidate) => {
        ...
        if (!title || title.length > 80 || byTitle.has(key) || /^(?:title|role|position|owner|department|team|name)$/i.test(title) || /(?:https?:|@|\/|\\|\b(?:true|false|null)\b)/i.test(title))
            return;
...
function normalizedSourceClaim(field, value) {
 ...
 */
export function createOrganizationFromEvidence(answers) {
    const byTitle = new Map();
    ...
    const sourceConflicts = ordered.flatMap((signal, signalIndex) => {
        const claimsByField = scalarClaims.get(canonicalRoleKey(signal.title));
        const scalar = ["department", "reportsTo"].flatMap((field) => {
        ...
        const authority = Array.from(authorityClaims.get(canonicalRoleKey(signal.title))?.values() ?? []).flatMap((claims) => {
            if (!claims.allow.size || !claims.deny.size)
                return [];
            const actionId = portableIdentifier(claims.action, "action", 60);
            return [{
...
export function knowledgeEvidence(sources, limit = 500) {
    return sources
        .filter((source) => source.content.trim())
        .filter((source) => !source.content.includes("<!-- deltadotta:start -->"))
        .slice(0, limit)
        ...
        return {
            ...
            kind: type === "codebase" ? "repository" : type === "database" ? "database" : "document",
...
export function applyFirstShiftReport(org, report) {
    const launchStatus = report.passed ? "preflighted" : "needs-refinement";
    return {
        ...org,
        updatedAt: "Just now",
