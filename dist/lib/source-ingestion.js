 ...
 */
export function mergeSourceScanResults(results, options = {}) {
    const maxFiles = options.maxFiles ?? 500;
    const maxTotalBytes = options.maxTotalBytes ?? 4_000_000;
    if (!Number.isInteger(maxFiles) || maxFiles < 1)
        throw new Error("combined source maxFiles must be a positive integer");
    if (!Number.isInteger(maxTotalBytes) || maxTotalBytes < 1)
        throw new Error("combined source maxTotalBytes must be a positive integer");
    const sources = [];
...
function sourceType(path, databaseHint) {
    const normalized = normalizedPath(path);
    const extension = extname(normalized).toLowerCase();
    if (databaseHint || databaseExtensions.has(extension) || sqliteDatabaseExtensions.has(extension) || /(?:^|\/)(?:database|db|migrations?|schema)(?:\/|$)/i.test(normalized))
        return "database";
    if (officeDocumentExtensions.has(extension) || archiveExtensions.has(extension))
        return "document";
...
function unsupportedReason(path, databaseHint) {
...
function displayPath(baseDirectory, absolutePath) {
...
function safeArchiveEntryPath(input) {
...
export async function extractSqliteSchema(path) {
    ...
    `).all();
        if (!rows.length)
            return "-- SQLite database contains no user-defined schema objects.";
 ...
 */
export async function collectKnowledgeSources(options) {
    ...
    async function visit(inputPath, databaseHint) {
        ...
        if (details.isDirectory()) {
            if (ignoredDirectories.has(basename(absolutePath)))
                return;
            let entries;
            try {
        ...
        if (archive) {
            ...
            const portableEntryPaths = new Set();
            let expandedBytes = 0;
            try {
                for (const entry of entries) {
                    ...
                    if (entryPaths.has(path))
                        throw new Error(`duplicate archive entry path: ${JSON.stringify(path)}`);
                    entryPaths.add(path);
                    ...
                    if (sourceType(path, databaseHint))
                        selected.push({ entry, path, declaredBytes: declaredBytes });
                }
            }
            catch (error) {
                skipped.push({
            ...
            try {
                for (const { entry, path, declaredBytes } of selected) {
                    ...
                    if (content.length !== declaredBytes)
                        throw new Error(`expanded size changed for archive entry ${JSON.stringify(path)}`);
            ...
            try {
                if (extension === ".pdf" && typeof globalThis.DOMMatrix === "undefined") {
                    ...
                    if (!runtimeProcess.getBuiltinModule) {
                        ...
                        runtimeProcess.getBuiltinModule = (id) => {
                            if (!isBuiltin(id))
                                throw new Error(`Not a built-in Node.js module: ${id}`);
    ...
    if (fileLimitExceeded)
        skipped.push({ path: "(scan)", reason: `stopped at the ${maxFiles}-file limit`, severity: "error" });
    if (totalLimitExceeded)
        skipped.push({ path: "(scan)", reason: `stopped at the ${maxTotalBytes}-byte total limit`, severity: "error" });
