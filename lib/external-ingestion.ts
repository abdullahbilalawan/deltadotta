...
import { collectKnowledgeSources, type SkippedSource, type SourceScanResult } from "./source-ingestion.js";
...
export type DatabaseDumpInvocation = {
  command: "pg_dump" | "mysqldump";
  args: string[];
...
function redact(input: string, secretValues: string[]) {
  return secretValues.filter(Boolean).reduce((value, secret) => value.split(secret).join("[redacted]"), input);
}

export function sanitizeExternalLocator(input: string) {
  try {
...
function contentExtension(url: URL, response: Response) {
  ...
  if (suppliedExtension) return suppliedExtension;
...
async function boundedResponseBody(response: Response, maxBytes: number) {
  ...
  try {
    while (true) {
      ...
      if (total > maxBytes) throw new Error(`response exceeded the ${maxBytes}-byte limit`);
...
function isSecureHttpSource(input: string) {
  const url = new URL(input);
  if (url.username || url.password) throw new Error("credentials in document URLs are not accepted; use --http-token-env");
  if (url.protocol === "https:") return url;
  if (url.protocol === "http:" && (url.hostname === "localhost" || url.hostname === "127.0.0.1" || url.hostname === "::1")) return url;
...
async function fetchWithSafeRedirects(
  ...
  for (let redirects = 0; redirects <= 5; redirects += 1) {
    ...
    if (redirects === 5) throw new Error("document URL exceeded the 5-redirect limit");
    const nextUrl = isSecureHttpSource(new URL(location, currentUrl).toString());
    if (nextUrl.origin !== currentUrl.origin) authorization = undefined;
    currentUrl = nextUrl;
  }
  throw new Error("document URL redirect failed");
}

async function collectHttpSource(
  input: string,
  options: Required<Pick<ExternalSourceOptions, "baseDirectory" | "maxBytesPerSource" | "maxBinaryBytes" | "timeoutMs">> & { httpTokenEnv?: string },
) {
  const requestedUrl = isSecureHttpSource(input);
  const token = options.httpTokenEnv ? process.env[options.httpTokenEnv] : undefined;
  if (options.httpTokenEnv && !token) throw new Error(`environment variable ${options.httpTokenEnv} is not set`);
  ...
  try {
    ...
    return {
      sources: scan.sources.map((source): RepositorySource => ({
        ...source,
...
function parseGitSpec(input: string) {
  const marker = input.lastIndexOf("#");
  if (marker <= 0) return { repository: input, ref: undefined };
  return { repository: input.slice(0, marker), ref: input.slice(marker + 1) || undefined };
}

function validateGitRepository(repository: string) {
  try {
    ...
    if (error instanceof Error && error.message.startsWith("credentials in HTTPS Git URLs")) throw error;
...
async function collectGitRepository(
  input: string,
  options: Required<Pick<ExternalSourceOptions, "baseDirectory" | "maxFiles" | "maxBytesPerSource" | "maxBinaryBytes" | "maxTotalBytes" | "timeoutMs">>,
) {
...
export function databaseDumpInvocation(input: string, environment: NodeJS.ProcessEnv = process.env): DatabaseDumpInvocation {
  const url = new URL(input);
...
async function collectDatabaseSchema(input: string, maxBytes: number, timeoutMs: number) {
  ...
  try {
    ...
    const content = result.stdout
      ...
      .join("\n")
      .trim();
    if (!content) throw new Error(`${invocation.command} returned an empty schema`);
    return {
      path: `${invocation.locator}#schema`,
      content,
      sourceType: "database" as const,
      sourceHash: evidenceHash(content),
      sourceEncoding: "text" as const,
...
function emptyCounts(): Record<SourceType, number> {
...
export async function collectExternalSources(options: ExternalSourceOptions): Promise<SourceScanResult> {
  ...
  function append(items: RepositorySource[], itemSkipped: SkippedSource[] = []) {
  ...
  const httpResults = await mapConcurrent(options.urls ?? [], httpConcurrency, async (input) => {
    ...
    try {
      ...
      return {
        sources: [] as RepositorySource[],
        skipped: [{ path: locator, reason: `HTTPS ingestion failed: ${safeError(error, [input])}`, severity: "error" as const }],
      };
    }
  });
