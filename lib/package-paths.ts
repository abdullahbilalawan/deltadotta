export function isGeneratedPackagePath(path: string) {
  ...
  if (/^roles\/[a-z0-9]+(?:-[a-z0-9]+)*\/SKILL\.md$/.test(normalized)) return true;
  if (/^contracts\/[a-z0-9]+(?:-[a-z0-9]+)*\.md$/.test(normalized)) return true;
