...
export type OrganizationReadinessReport = {
  schemaVersion: "1.0";
  organization: string;
  provider: string;
...
export function evaluateOrganizationReadiness(
  ...
  const recognizedFingerprint = (value: string | undefined) => Boolean(
    value && /^(?:sha256-[a-f0-9]{64}|fnv1a-[a-f0-9]{8})$/i.test(value),
  );
  ...
  const cycleRoles = new Set<string>();
  ...
  for (const role of organization.roles) {
    ...
    if (cyclic) cycleRoles.add(role.id);
  }
  add({
    id: "reporting-cycles",
  ...
  if (files) {
    ...
    try {
      const parsed = JSON.parse(files["validation/generated-files.json"] ?? "") as Record<string, unknown>;
    ...
    try {
      const parsed = JSON.parse(files["validation/source-ingestion.json"] ?? "") as Record<string, unknown>;
    ...
    type KnowledgeFileRecord = { path?: unknown; bytes?: unknown; sha256?: unknown };
    type ProviderKnowledgeRecord = {
      maxBytesPerKnowledgeFile?: unknown;
      maxProjectFiles?: unknown;
      reservedProjectFiles?: unknown;
      projectFileCount?: unknown;
      capacity?: unknown;
      files?: unknown;
      handoffFiles?: unknown;
      roleIds?: unknown;
      evidenceIds?: unknown;
    };
    ...
    const missingRoleTitles: string[] = [];
    const missingEvidenceIds: string[] = [];
    try {
      const parsed = JSON.parse(files["validation/provider-knowledge.json"] ?? "") as {
        schemaVersion?: unknown;
        providers?: Record<string, ProviderKnowledgeRecord>;
      };
      ...
      for (const provider of ["chatgpt", "claude"] as const) {
        ...
        const records = Array.isArray(manifest?.files) ? manifest.files as KnowledgeFileRecord[] : [];
        ...
        const handoffRecords = Array.isArray(manifest?.handoffFiles)
          ? manifest.handoffFiles as KnowledgeFileRecord[]
          : [];
        ...
        manifestsValid = manifestsValid
          ...
          && records.every((record) => {
            if (typeof record.path !== "string" || typeof record.bytes !== "number" || typeof record.sha256 !== "string") return false;
        ...
        const roleRecords = new Map(Array.from(
          combinedKnowledge.matchAll(/^## (.+)\n- Role id: (.+)$/gm),
          (match) => [match[2].trim(), match[1].trim()],
        ));
        const evidenceRecords = new Set(Array.from(
          combinedKnowledge.matchAll(/^- Evidence id: (.+)$/gm),
          (match) => match[1].trim(),
        ));
        organization.roles.forEach((role) => {
          if (roleRecords.get(role.id) !== role.title) missingRoleTitles.push(`${label}: ${role.title}`);
        });
        organization.evidence.forEach((evidence) => {
          if (!evidenceRecords.has(evidence.id)) missingEvidenceIds.push(`${label}: ${evidence.id}`);
        });
...
export function readinessMarkdown(report: OrganizationReadinessReport) {
